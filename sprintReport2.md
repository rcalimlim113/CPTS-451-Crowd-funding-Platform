 ...
 * Database Operations Tested for CRUD with Real Scenarios
 * Advanced Retrieval Queries
 * Sample Data Validation

## Work Summary (Developer Facing)
For this sprint, in order to complete the above issues, we first had to make a UI for our database, which was done using FLASK. After our UI was set up, we went made multiple business rules and discussed how they would be enforced. We then checked for edge cases in multiple scenarios and made sure the system would be able to handle them. The entire database was then normalizaed to Fifth Normal Form. Finally, the entire database and UI were tested using CRUD with real scenarios and real-world advanced retrieval queries. The biggest difficulty was correctly converting the entire database to 5NF and making the UI in FLASK.
